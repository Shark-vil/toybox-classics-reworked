...
function SWEP:GetPrimaryDelay()
  local tempo = GUNSTRU:RetreiveTempo(self:GetOwner())
...
function SWEP:ShootEffects()
  ...
  net.Start("ToyBoxReworked_GUNSTRU_CouldShoot")
  net.Send(self:GetOwner())
end

function SWEP:FireA()
  local num_bullets = 1
  local damage = self.Primary.DamageBase * self.DamageMul * GUNSTRU:GetHitRatio(self:GetOwner(), 1) * self:GetPrimaryDelay()
  local aimcone = 0
  local bullet = {}
...
function SWEP:FireB()
  local num_bullets = 1
  local damage = self.Secondary.DamageBase * self.DamageMul * GUNSTRU:GetHitRatio(self:GetOwner(), 1) * self:GetPrimaryDelay()
  local aimcone = 0
  local bullet = {}
  bullet.Num = num_bullets
  bullet.Src = self:GetOwner():GetShootPos()
...
function SWEP:Reload()
  ...
  self:ApplyCursorPos()
  local deltaTime = CurTime() - (self:GetOwner().GS and self:GetOwner().GS.ReloadTime or 0)

  if deltaTime > 1 then
    GUNSTRU:RequirePatternUpdate(self:GetOwner()) -- This has to go first
    self:GetOwner().GS.ReloadTime = CurTime()
  end
...
function SWEP:Think()
  if CLIENT then
    ...
    if (GetConVarNumber("gs_rchold") == 0) and self:GetOwner():KeyReleased(IN_ATTACK2) then
      GUNSTRU:CloseMenu()
    end
  ...
  if SERVER then
    if self.NeedsInit then
      ...
      timer.Simple(1.5, function(self)
        if IsValid(self) then
          GUNSTRU:RequirePatternUpdate(self:GetOwner())
        end
      end, self)
    end
  end
...
function SWEP:IncrementCursorPos()
  local sPattern = GUNSTRU:GetPattern(self:GetOwner(), 1)
...
function SWEP:PrimaryPattern()
  return self:PatternPlay(self:GetOwner(), 1, true)
end

function SWEP:HarmonicsPattern()
  return self:PatternPlay(self:GetOwner(), 2, true)
end
...
function GUNSTRU:Gunstrumentalize(tab, harmonics)
  ...
  timer.Simple(iPattern * delay, function()
    chat.AddText(Color(255, 255, 0), "Finished dumping...")
  end)

  timer.Simple(iPattern * delay + 2, function()
    chat.AddText(Color(0, 255, 0), "Done loading!", Color(255, 255, 255), " You should now ", Color(255, 255, 0), "reload your weapon by hitting your Reload key", Color(255, 255, 255), " (just as if you were reloading your SMG).")
  end)
end
...
if CLIENT then
  ...
  function SWEP:DrawPatterns()
    ...
    local offs = GUNSTRU:RetreiveGlobalOffsets(self:GetOwner())
    ...
    if PATTERN_DEBUGMODE then
      ...
      surface.DrawTexturedRectRotated(ScrW() / 2, 64, 8, 256, 0)
      draw.SimpleText(self:RetreiveCursorPos(), "DebugFixed", ScrW() / 2 + 1, 17, PATTERN_COLORNONE[1], TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
      draw.SimpleText(self:RetreiveCursorPos(), "DebugFixed", ScrW() / 2, 16, PATTERN_COLORDEBUG[1], TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    end
    ...
    for n = 1, GUNSTRU.NumPatterns do
      ...
      if PATTERN_DEBUGMODE then
        ...
        surface.DrawTexturedRectRotated(ScrW() / 2, yImpossible, 128, 8, 0)
        surface.SetDrawColor(PATTERN_COLIMPOSSIBLE[1])
        surface.DrawTexturedRectRotated(ScrW() / 2, yImpossible, 16, 16, CurTime() * 30)
      end
      ...
      for i = math.max(1, pos - PATTERN_VISRANGEPASS), math.min(pos + PATTERN_VISRANGEPRE, patternLength / 2 + 1) do
        ...
        if high then
          ...
          surface.DrawTexturedRectRotated(xPos, yPos, ScrH() / 64 * ((isPre and 2 or 1.5) * ashrive + add * 2), ScrH() / 64 * ((isPre and 2 or 1.5) * ashrive + add * 2), 45 + add * 180)

          if PATTERN_DEBUGMODE then
            draw.SimpleText(sNote, "DebugFixed", xPos + 1, yPos + 9, PATTERN_COLORNONE[1], TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
            draw.SimpleText(sNote, "DebugFixed", xPos, yPos + 8, isPossible and (isPre and PATTERN_COLPRE[n] or PATTERN_COLPASS[n]) or PATTERN_COLIMPOSSIBLE[1], TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
          end
        end
      end
    end
  ...
  net.Receive("ToyBoxReworked_GUNSTRU_CouldShoot", function()
...
if CLIENT then
  ...
  function GUNSTRU:BuildContents()
    ...
    do
      ...
      header.CTS[item].DoClick = function(self)
        GUNSTRU:CloseMenu()
      end
    end
    ...
    do
      ...
      do
        ...
        form.CTS[item].Texts = {"HELP ME! My weapon doesn't shoot anything!", "READ THIS: Clicking this again will reset your settings.", "DONE. Do you also want to reset your music sheet?", "DONE."}
    ...
    do
      ...
      do
        ...
        table.insert(sets, {"Gunstrumental", "gunstrumental/gunstru__1.wav", 0, "gunstrumental/musicbox__2.wav", -4})

        table.insert(sets, {"Gunstrumental (Higher 2,2)", "gunstrumental/gunstru__1o880.wav", 0, "gunstrumental/musicbox__2o880.wav", -4})

        table.insert(sets, {"Gunstrumental (Higher 3,2)", "gunstrumental/gunstru__1o1760.wav", 0, "gunstrumental/musicbox__2o880.wav", -4})

        table.insert(sets, {"Piano", "gunstrumental/piano_f3__1.wav", 0, "gunstrumental/piano_f3__2.wav", 0})

        table.insert(sets, {"Piano (Higher 2,2)", "gunstrumental/piano_f3__1o880.wav", 0, "gunstrumental/piano_f3__2o880.wav", 0})

        table.insert(sets, {"Piano (Higher 3,3)", "gunstrumental/piano_f3__1o1760.wav", 0, "gunstrumental/piano_f3__2o1760.wav", 0})

        table.insert(sets, {"Bonk", "vo/scout_specialcompleted03.mp3", 0, "vo/scout_specialcompleted02.mp3", -1})

        table.insert(sets, {"Music Box", "gunstrumental/musicbox__1.wav", 0, "gunstrumental/musicbox__2.wav", 0})

        table.insert(sets, {"Music Box (Higher 2,2)", "gunstrumental/musicbox__1o880.wav", 0, "gunstrumental/musicbox__2o880.wav", 0})

        table.insert(sets, {"SMG / AR2", "weapons/smg1/smg1_fire1.wav", 0, "weapons/ar2/fire1.wav", 0})

        table.insert(sets, {"TF2 Bat / SMG", "weapons/bat_hit.wav", 0, "weapons/smg_shoot.wav", 0})

        table.insert(sets, {"Triangle waves", "gunstrumental/gs_tri_03s__1.wav", 0, "gunstrumental/gs_tri_03s__2.wav", 0})

        table.insert(sets, {"DTMF Phone dial", "gunstrumental/dtmf1__1.wav", 0, "gunstrumental/dtmf1__2.wav", 0})

        table.insert(sets, {"NES Set #1 (DH/BASS)", "gunstrumental/nes_test_1__1.wav", 0, "gunstrumental/nes_test_2__2.wav", 0})

        table.insert(sets, {"NES Set #2 (DH/DH)", "gunstrumental/nes_test_1__1.wav", 0, "gunstrumental/nes_test_1__2.wav", 0})

        table.insert(sets, {"Vibraphone", "gunstrumental/vibraphone__1.wav", 0, "gunstrumental/vibraphone__2.wav", 0})

        table.insert(sets, {"Vibraphone (Higher 2,2)", "gunstrumental/vibraphone__1o880.wav", 0, "gunstrumental/vibraphone__2o880.wav", 0})

        table.insert(sets, {"Vibraphone (Higher 3,3)", "gunstrumental/vibraphone__1o1760.wav", 0, "gunstrumental/vibraphone__2o1760.wav", 0})

        table.insert(sets, {"Music Box/Vibraphone (Higher 2,2)", "gunstrumental/musicbox__1o880.wav", 0, "gunstrumental/vibraphone__2o880.wav", 1})

        table.insert(sets, {"Vibraphone/Music Box (Higher 2,2)", "gunstrumental/vibraphone__1o880.wav", 1, "gunstrumental/musicbox__2o880.wav", 0})
    ...
    do
      ...
      do
        local item = "help"
        form.CTS[item] = vgui.Create("DLabel", form)
        form.CTS[item]:SetText([[Create your own music here! (You should be using a text editor at the same time to perform safe saves)
...
Please note that if your pattern is short, reloading your weapon may take less time.]])
    ...
    do
      ...
      do
        ...
        form.CTS[item].Think = function(self)
          form.CTS[item]:SetValue(string.sub(GUNSTRU:GetPattern(LocalPlayer(), 1), self.form.CTS["trimstart"]:GetValue() * 2 - 1, self.form.CTS["trimend"]:GetValue() * 2) .. "\n" .. string.sub(GUNSTRU:GetPattern(LocalPlayer(), 2), self.form.CTS["trimstart"]:GetValue() * 2 - 1, self.form.CTS["trimend"]:GetValue() * 2))
        end
      ...
      do
        ...
        form.CTS[item].Think = function(self)
          form.CTS[item]:SetValue((self.form.CTS["trimend"]:GetValue() - self.form.CTS["trimstart"]:GetValue() + 1) * 2)
        end
    ...
    do
      ...
      do
        ...
        form.CTS[item].Think = function(self)
          ...
          form.CTS[item]:SetValue((first[1] or "") .. rep .. (second[1] or "") .. "\n" .. (first[2] or "") .. rep .. (second[2] or ""))
        end
      ...
      do
        ...
        form.CTS[item].Think = function(self)
          ...
          form.CTS[item]:SetValue(alen .. " - " .. blen .. " = " .. diff)
        end
